...
def build_context_from_table(
    ...
    for i in range(data.shape[0]):
        if has_capacity: 
            # Challenge #3: update config.CONSULTANT_FULL_CAPACITY with the selected capacity  
            if int(data.loc[i, config.CONSULTANT_ATTR_CAPACITY]) < config.CONSULTANT_FULL_CAPACITY: 
