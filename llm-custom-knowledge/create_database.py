...
def add_mockup_data_to_database(conn: sqlite3.Connection) -> None:
  """Function to add mockup data to database."""
  cursor = conn.cursor()
...
def create_database(db_path: str) -> None:
    """Creates the database and populates it with mockup data."""
    conn = sqlite3.connect(db_path)
    add_mockup_data_to_database(conn)
    conn.close()

# Create the database in the specified folder
create_database("data/travel_information.sqlite")
